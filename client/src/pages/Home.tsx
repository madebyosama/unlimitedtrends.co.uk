...
function Evidence() {
  ...
  return <section className="section-light evidence-section" id="evidence"><div className="section-shell"><SectionMeta code="SEC 09" children="EVIDENCE / COMPLIANCE" /><div className="evidence-layout"><div><h2>Specific claims.<br /><em>Retained evidence.</em></h2><div className="evidence-table"><div className="evidence-table-head"><span>TRUST SIGNAL</span><span>WHAT WE DO</span><span>EVIDENCE WE PROVIDE</span></div>{rows.map((row) => <div className="evidence-table-row" key={row[0]}><strong>{row[0]}</strong><span>{row[1]}</span><span>{row[2]}</span></div>)}</div></div><aside className="evidence-pack"><ShieldCheck size={25} strokeWidth={1.5} /><span className="data-label">SUPPLIER PACK / REQUEST</span><h3>Ask for the<br /><em>evidence pack.</em></h3><p>We can share the practical records that support the way we buy, review, collect and settle.</p><a className="text-link" href="mailto:Help@unlimitedtrends.co.uk?subject=Supplier evidence pack">Request supplier pack <ArrowUpRight size={17} /></a></aside></div></div></section>;
}
